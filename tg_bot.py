...
TEMPLATES = {
    ...
    "file_error": {
        ...
        "not_exist": (
            "This file does not exist. Choose an existing file."
            "I already have the following files:\n{files_list}"
        ),
    },
    "wrong_command": (
        "Something get wrong. " "Did you enter the command correctly?\n" "Try again!"
    ),
    "done": "Done!",
...
def new_file(updater, context):
    ...
    if file_name in listdir("./files/"):
        updater.message.reply_text(
            TEMPLATES["file_error"]["already_exist"].format(
...
def add_to_file(updater, context):
    ...
    if file_name in listdir("./files/"):
        ...
        updater.message.reply_text(
            TEMPLATES["file_error"]["not_exist"].format(
                files_list=str(listdir("./files"))[1:-1]
            )
        )
...
def image(updater, context):
    ...
    visual_recognition = VisualRecognitionV3(
        version=IBM_VERSION, authenticator=authenticator
    )
    ...
    with open("./images/img.jpg", "rb") as image_file:
        classes = visual_recognition.classify(
            images_file=image_file, threshold="0.6"
        ).get_result()
    ...
    updater.message.reply_text(
        TEMPLATES["image_recognition"].format(
